...
export const SECTION_TYPE_MAPPINGS: Record<string, string> = {
  ...
  "embauche.clauses-particulieres": "embauche.clauses-particulieres",
  ...
  // Protection sociale
  "protection-sociale.prevoyance": "protection-sociale.prevoyance",
  "protection-sociale.retraite": "protection-sociale.retraite",
  "protection-sociale.mutuelle": "protection-sociale.mutuelle",
  "protection-sociale.complementaire-sante": "protection-sociale.mutuelle",
  ...
  "rupture.depart-volontaire-retraite": "rupture.retraite",
  ...
  // Mappage des variations courantes dans les noms de sections
  "delai-prevenance": "embauche.delai-prevenance",
  "periode-essai": "embauche.periode-essai",
  "duree-travail": "temps-travail.duree-travail",
  "heures-supplementaires": "temps-travail.heures-sup",
  "temps-partiel": "temps-travail.temps-partiel",
  "travail-temps-partiel": "temps-travail.temps-partiel",
  "forfait-jours": "temps-travail.forfait-jours",
  "conges-payes": "conges.conges-payes",
  "indemnite-licenciement": "rupture.licenciement",
  "preavis-licenciement": "rupture.preavis",
  "preavis-demission": "rupture.preavis",
  "travail-nuit": "temps-travail.travail-nuit",
  "travail-de-nuit": "temps-travail.travail-nuit",
...
/**
 * Récupère le type de section mappé si disponible
 * @param sectionType - Type de section original
 * @returns Le type de section mappé ou le type original si pas de mapping
 */
export const getMappedSectionType = (sectionType: string): string => {
  return SECTION_TYPE_MAPPINGS[sectionType] || sectionType;
};
...
- Des dispositions plus favorables peuvent être prévues par la convention collective ou un accord d'entreprise.`,
  ...
  "absences.maternite-paternite": `# Congé Maternité 

## Durée du Congé Maternité 

| Situation de la mère                           | Congé prénatal | Congé postnatal | Total           |
|-----------------------------------------------|----------------|-----------------|-----------------|
| Naissance d'un premier ou deuxième enfant     | 6 semaines     | 10 semaines     | 16 semaines     |
| Naissance d'un troisième enfant ou plus       | 8 semaines     | 18 semaines     | 26 semaines     |
...
- **Montant** : L'indemnité journalière est égale au salaire journalier de base, calculé sur la moyenne des 3 derniers mois, dans la limite du plafond de la Sécurité sociale.
- **Conditions** : Avoir travaillé au moins 150 heures au cours des 3 mois civils précédant l'arrêt ou avoir cotisé sur une rémunération au moins égale à 1 015 fois le SMIC horaire durant les 6 mois précédents.
...
- **Congé de naissance** : 3 jours (à prendre à la naissance de l'enfant)
- **Congé paternité** : 25 jours calendaires (32 jours en cas de naissances multiples)
  - **Période obligatoire** : 4 jours consécutifs immédiatement après le congé de naissance
  - **Période facultative** : 21 jours (ou 28 jours en cas de naissances multiples) à prendre dans les 6 mois suivant la naissance

## Indemnisation 

- **Congé de naissance** : Maintien du salaire par l'employeur
- **Congé paternité** : Indemnités journalières versées par la Sécurité sociale, sous les mêmes conditions que pour le congé maternité.`,
...
/**
 * Vérifie si une section possède un dispositif légal de référence
 * @param sectionType - Type de section (format: "categorie.sous-categorie")
 */
export const hasDispositifLegal = (sectionType: string): boolean => {
  // Vérifier d'abord si la section a un dispositif direct
  if (DISPOSITIFS_LEGAUX[sectionType] !== undefined) {
    return true;
  }
  
  // Vérifier si la section a un mapping vers un dispositif légal
  const mappedType = getMappedSectionType(sectionType);
  return mappedType !== sectionType && DISPOSITIFS_LEGAUX[mappedType] !== undefined;
};

/**
 * Récupère le dispositif légal pour une section donnée
 * @param sectionType - Type de section (format: "categorie.sous-categorie")
 * @returns Le texte du dispositif légal ou une chaîne vide
 */
export const getDispositifLegal = (sectionType: string): string => {
  ...
  // Essayer de récupérer via le mapping
  const mappedType = getMappedSectionType(sectionType);
  return DISPOSITIFS_LEGAUX[mappedType] || "";
};